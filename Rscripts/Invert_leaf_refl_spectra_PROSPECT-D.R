...
## Output directory
dataID <- "Panama2016"
out.dir <- file.path("~/Data/Dropbox/MANUSCRIPTS/BNL_TEST/canopyRT/","R_Output",dataID,"PROSPECTD",
                     'Range_400_700nm')
...
if (custom_prior) {
  ...
  #custom_prior <- list(Cab = list('Cab', 'lnorm', log(3.6683201), 0.1952258, 1))
  #prior <- prospect_bt_prior("D", custom_prior=custom_prior)
} else {
  prior <- prospect_bt_prior("D")
}
...
for (i in 1:6 ) {
  ...
  refl <- spectral.response(as.vector(prospect(param = mod.params[i,c(1,4,7,10,13,16,19)], version=prospect_ver)[,1]), 'licor')
  trans <- spectral.response(as.vector(prospect(param = mod.params[i,c(1,4,7,10,13,16,19)], version=prospect_ver)[,2]), 'licor')
